 ...
 *
 * GET /api/auth/google
 *   - Without ?callback=true: redirects to Google OAuth consent screen
...
import { getGoogleAuthUrl, handleGoogleCallback } from '@/lib/auth/sso';
  ...
  if (!isCallback || !code) {
    try {
      ...
      return NextResponse.json(
        {
          error: error instanceof Error ? error.message : 'Failed to build Google auth URL',
        },
        { status: 500 },
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (existing) {
        ...
        ).run(googleUser.name ?? null, googleUser.avatarUrl ?? null, existing.id);

        user = {
          id: existing.id,
          email: existing.email,
          name: googleUser.name ?? existing.name ?? undefined,
          avatarUrl: googleUser.avatarUrl ?? existing.avatar_url ?? undefined,
        ...
        if (membership) {
          ...
          db.prepare('INSERT INTO teams (id, name, slug) VALUES (?, ?, ?)').run(
            teamId,
            `${googleUser.name ?? 'My'}'s Team`,
            `${slug}-${teamId.slice(0, 8)}`,
          );
          db.prepare(
            "INSERT INTO team_members (id, team_id, user_id, email, name, role) VALUES (?, ?, ?, ?, ?, 'owner')",
          ).run(crypto.randomUUID(), teamId, existing.id, existing.email, googleUser.name ?? null);
        ...
        ).run(userId, googleUser.email, googleUser.name ?? null, googleUser.avatarUrl ?? null, now);

        // Create personal team
        teamId = crypto.randomUUID();
        const slug = googleUser.email.split('@')[0]!.replace(/[^a-z0-9-]/gi, '-').toLowerCase();
        ...
        db.prepare(
          "INSERT INTO team_members (id, team_id, user_id, email, name, role) VALUES (?, ?, ?, ?, ?, 'owner')",
        ).run(crypto.randomUUID(), teamId, userId, googleUser.email, googleUser.name ?? null);
